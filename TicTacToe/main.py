...
#demo board shows you the current state of the board
def demoboard():
...
#functions to change symbol for every steps    
def pressed(value):
    ...
    #if type isn't 0 allows computer to take a move
...
#this function allows computer to make a move     
def compmove():
    global symble, move_no
    choice = 0
    
    #in easy move its just computer taking moves randomly
    if type == 1:
        choice = 0
        ...
        #in normal mode computer trys to take all the good moves but don't blocks the winning possition
        ...
        if type == 3:
                ...
                #in hard mode computer tries to block players winning possition
                if numbers[0]==numbers[1] and numbers[2].isdigit() is True:
                    ...
                    choice = 4    
                elif numbers[0]==numbers[3] and numbers[6].isdigit() is True:
                    choice = 6
...
#this function resets all board to the starting possitions           
def reset():
...
#this returns true if somemone makes the winnning move else false
def win_check():
...
#whole function creates and manages the game board 
def board():
    for widgets in frame.winfo_children():
        widgets.destroy()
    for widgets in frame1.winfo_children():
...
def welcome():
    ...
    def show_level():
        ...
        easy_button = Button(frame, text="EASY",font=("Times New Roman", 15),bg='#967852',width=32, command=lambda: game_mode(1),pady=30).grid(row =0 , column=0,padx=7, pady =10)
        normal_button = Button(frame, text="NORMAL",font=("Times New Roman", 15),bg='#967852',width=32, command=lambda: game_mode(2),pady=30).grid(row =1 , column=0,padx=7, pady=10)
        easy_button = Button(frame, text="HARD",font=("Times New Roman", 15),bg='#967852',width=32, command=lambda: game_mode(3),pady=30).grid(row =2 , column=0,padx=7,pady= 10)
    ...
    Human_button = Button(frame, text="Human",font=("Times New Roman", 15),bg='#967852',width=12 ,pady=9, command= lambda: game_mode(0)).grid(row =1 , column=1,padx=5,pady=10)
    Computer_button = Button(frame, text="Computer",font=("Times New Roman", 15),bg='#967852',width=12,pady=9, command= show_level).grid(row =1 , column=2,padx=5,pady=10)
